...
| `tissue doctor` / `tissue smoke` | Environment and bounded smoke checks; `doctor` exits non-zero when the deployed agents are missing, invalid, or drifted, or when the registry mount assertion fails |
