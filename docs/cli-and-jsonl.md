...
| Command | Purpose |
|---|---|
| `tissue daemon` | Resident A′ reconcile loop under s6; seeds resident-dependency health from the reconcile result, re-probes the endpoint every iteration, and while the resident OpenCode service is unavailable still polls/ingests, promotes `READY`→`QUEUED`, and executes effects but performs no triage/claim/ensure-resolution/relay (one `daemon.dependency_gated` warn per pass), resuming automatically once the probe observes the service restored — no restart |
| `tissue tick` / `tissue reconcile` | One shared P0–P6 pass; authoritative at P2 — the session census applies durable recovery and derives resident dependency health, gating P6 prompt-dependent resume when the resident census fails. A failed P2 still yields a non-zero exit while safe local reconciliation (P1/P3/P4/P5) runs |
